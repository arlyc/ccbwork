...
def check_process(process):
    command = 'wmic process get name |findstr /i "%s"' % process
...
if __name__ == '__main__':
    ...
    if false_count == 0:
        print True
        Loger().write_line('process', 'True')
    else:
        print str(result)
        Loger().write_line('process', str(result))